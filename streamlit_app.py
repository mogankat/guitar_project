...
SVG_H     = MT + (N_STRINGS - 1) * STRING_H + 60
...
def note_at(string, fret):
    return NOTES[(NOTES.index(STRING_NOTES[string]) + fret) % 12]
...
def find_triad_voicings(positions, required_labels):
    ...
    by_string = {i: [] for i in range(N_STRINGS)}
    for p in positions:
...
def svg_rect(x1, y1, x2, y2, color, stroke_w=2):
    return (
    ...
    for string in range(N_STRINGS):
        for fret in range(N_FRETS):
            ...
            if ni in degrees_dict:
                ...
                freq = OPEN_STRING_FREQS[string] * (2 ** (fret / 12))
...
def render_triads_only():
    ...
    for group, voicing in get_active_voicings():
        for p in voicing:
            ...
            freq = OPEN_STRING_FREQS[p['string']] * (2 ** (p['fret'] / 12))
...
def render_caged():
    ...
    for shape_name, shape in CAGED_SHAPES.items():
        if shape_name not in s.caged:
            continue
        open_ni   = NOTES.index(STRING_NOTES[shape['root_string']])
        ...
        for si, offset in enumerate(shape['pattern']):
            ...
            if exp_fret > 22:
                continue
            open_sn = NOTES.index(STRING_NOTES[si])
            ...
            freq = OPEN_STRING_FREQS[si] * (2 ** (best_fret / 12))
            parts.append(svg_circle(fx(best_fret), sy(si), color, display, freq))
        ...
        if frets_drawn:
            ...
            parts.append(svg_rect(min(xs)-15, min(ys)-15, max(xs)+15, max(ys)+15, bc, 2))
            parts.append(
...
def build_svg():
    ...
    for i in range(N_FRETS):
        x = fx(i)
        w = 3 if i == 0 else 1
        p.append(f'<line x1="{x}" y1="{sy(0)}" x2="{x}" y2="{sy(N_STRINGS-1)}" '
                 f'stroke="{fc}" stroke-width="{w}"/>')

    for fret in [3, 5, 7, 9, 15, 17, 19, 21]:
        p.append(f'<circle cx="{fx(fret)}" cy="175" r="5" fill="{mc}"/>')
    p.append(f'<circle cx="{fx(12)}" cy="165" r="5" fill="{mc}"/>')
    p.append(f'<circle cx="{fx(12)}" cy="185" r="5" fill="{mc}"/>')
...
def main():
    ...
    st.components.v1.html(build_fretboard_html(), height=SVG_H + 20, scrolling=False)
    ...
    for i, note in enumerate(NOTES):
        if root_cols[i + 1].button(note, key=f"r_{note}",
                                   type="primary" if note == s.root else "secondary",
    ...
    triad_cols   = st.columns([1, 1.2] + [1] * 4)
